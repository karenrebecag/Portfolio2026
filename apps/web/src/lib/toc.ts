...
function initTableOfContentsRoot(root: HTMLElement): (() => void) | undefined {
  ...
  listEl.querySelectorAll('[data-toc-item]').forEach((el) => el.remove())

  let templateLink = listEl.querySelector<HTMLAnchorElement>('[data-toc-link]')
  if (!templateLink) {
    const stored = root.getAttribute('data-toc-template-stored')
    if (stored) {
      listEl.insertAdjacentHTML('beforeend', stored)
      templateLink = listEl.querySelector<HTMLAnchorElement>('[data-toc-link]')
    }
  }

  if (!templateLink) return

  if (!root.hasAttribute('data-toc-template-stored')) {
    root.setAttribute('data-toc-template-stored', templateLink.outerHTML)
  }
  ...
  if (ST) {
    ...
    cleanups.push(() => cancelAnimationFrame(initialRaf))
  } else {
    const onScroll = () => {
      let activeIndex = 0
  ...
  const onClick = (e: Event) => {
    ...
    const lenis = typeof window !== 'undefined' ? (window as Window & { lenis?: { scrollTo?: (el: HTMLElement, opts: { offset: number }) => void } }).lenis : undefined
  ...
  cleanups.push(() => listEl.removeEventListener('click', onClick))
  cleanups.push(() => listEl.querySelectorAll('[data-toc-item]').forEach((el) => el.remove()))
...
export function initTableOfContents(): () => void {
  ...
  document.querySelectorAll<HTMLElement>('[data-toc-wrap]').forEach((root) => {
    const cleanup = initTableOfContentsRoot(root)
