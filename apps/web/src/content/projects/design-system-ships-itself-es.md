...
Llegué al cliente en un momento preciso. El equipo de producto, un grupo de gente muy talentosa, estaba terminando la primera etapa de su design system: un sistema con la estética de shadcn, construido para la plataforma del cliente. Buen trabajo, base sólida. Pero vivía dentro del producto.
...
El Client UIKit no es una copia del sistema de producto. Es una reinterpretación, mismo lenguaje visual pero arquitectura distinta, optimizada para dos lectores al mismo tiempo: el developer y el LLM.
...
```tree Estructura del monorepo
{
  ...
  "folders": [
    ...
    { "category": "cli", "path": "cli/", "files": ["CLI de auth + consumo del MCP"] },
    { "category": "layouts", "path": "layouts/", "files": ["plantillas de página, patrón pure-DS"] }
  ]
}
```
...
El design system no es un solo repositorio; son ==cinco repos coordinados== con contratos escritos en cada `CLAUDE.md`. Esta tabla es el índice que uso al onboardar ingeniería o agentes.
...
// client.discovery: lo que el agente puede SABER
{
...
// client.implementation: lo que el agente necesita para HACER
{
  baseClass,      // clase raíz real, ej. "button"
  cssClasses,     // todos los nombres BEM
  peerDeps,       // ej. "gsap"
  hasCss, hasReact
  // accesible SOLO vía client_uikit_source
}
```
...
graph TD
  subgraph L1["1. Minimización de datos"]
    COMP["client_uikit_component, discovery: sin CSS, sin baseClass"]
  end
  subgraph L2["2. Autoridad única"]
    SRC["client_uikit_source, el ÚNICO tool con CSS real"]
  end
  subgraph L3["3. Validación"]
    VAL["client_uikit_validate, detecta hex inventados, clases desconocidas"]
  end
...
```mermaid Arquitectura plataforma MCP Client UIKit
flowchart TB
  ...
  subgraph AuthPlane["Plano auth, solo empresa"]
    ...
    DocsApp["App docs Next.js<br/>uikit.client.io"]
...
4. Tras el login, la app valida **email corporativo**: en nuestro deploy, `primaryEmailAddress` debe terminar en `@client.io`. El resto ve *Access Denied*; no se emite código.
5. El frontend mintea un authorization code de vida corta (JWT, issuer `client-uikit-mcp-oauth`, embebe `code_challenge`) y redirige al `redirect_uri` del cliente.
...
4. La CLI hace `POST` del code a `/api/auth/mcp-token`, recibe el JWT y lo guarda en `~/.config/client-uikit/credentials.json`.
...
- **Fail-closed por default.** El sistema no debería depender de que el agente "se porte bien". Debería hacer que el error correcto sea el único camino disponible. La señal `requires_client_uikit_source` no le pide al modelo que sea responsable; le quita la opción de no serlo.
