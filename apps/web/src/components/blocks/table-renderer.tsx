  ...
  if (hex) {
    return (
      <span className="inline-flex items-center gap-2 min-w-0">
        ...
        <code className="text-2xs font-accent text-muted-foreground uppercase tracking-wide">{hex}</code>
      </span>
    )
  }
            ...
            <thead>
              <tr className="bg-muted">
                {headers.map((h, i) => (
                  <th
                    key={i}
                    className="px-4 py-2.5 text-left text-2xs font-bold uppercase tracking-widest font-accent text-muted-foreground whitespace-nowrap border-b border-border"
                  >
